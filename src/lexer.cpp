...
void Lexer::_number() {
	...
	if(decimal) _add_token(TOKEN_DOUBLE);
	else _add_token(TOKEN_INT);
}
...
void Lexer::_identifier_type() {
	std::string identifier = _source.substr(_start_pos, _current_pos - _start_pos);

	if(_token_map.find(identifier) == _token_map.end()) {
		_add_token(TOKEN_ID);
	}
	else _add_token(_token_map[identifier]);
}
...
bool Lexer::_at_end() {
	return _current_pos >= _source.size();
}
